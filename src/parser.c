...
void consume(TAtom atom) {
  if (lookahead == atom) {
    ...
    // {
    // case IDENTIFICADOR:
    //   printf("\n# %2d:identifier : %s", info_atom.line,
    //   info_atom.attribute.id); break;

    // case NUMERO:
    //   printf("\n# %2d:number : %d", info_atom.line,
    //   info_atom.attribute.number); break;

    // default:
    //   printf("\n# %2d:%s", info_atom.line, print_atom(lookahead)); // Imprime
    //   o token ATUAL break;
    // }
    ...
    printf("\n# %2d:Erro sintatico: esperado [%s] encontrado [%s]\n",
           info_atom.line, print_expected_atom(atom),
           print_expected_atom(lookahead));
    exit(1);
  }
}
...
void variable_declaration_part() {
  ...
  if (lookahead == VAR) {
    ...
    printf("\nAMEM %d",
           countSymbols(&tabelaSimbolos)); // Aloca espaço para as variáveis + 1
                                           // (base da pilha)
  }
...
// <statement> ::=   <assignment_statement>  |  <read_statement> |
// <write_statement> | <if_statement> | <while_statement> | <statement_part>
void statement() {
...
void read_statement() {
  ...
  printf("\nARMZ %d", endereco); // MEPA - Instrução de armazenamento após a
                                 // função read

  consume(IDENTIFICADOR);

  if (searchSymbol(&tabelaSimbolos, id) ==
      NULL) { // Verifica se a variável foi declarada na tabela de símbolos
    printf("\n# %2d:Erro semantico: variavel '%s' nao declarada\n",
           info_atom.line, id);
    exit(1);
  }
  ...
  while (lookahead == VIRGULA) {
    ...
    printf("\nLEIT"); // MEPA - Instrução de leitura
    printf("\nARMZ %d",
           endereco); // MEPA - Instrução de armazenamento após a função read
  }
...
void write_statement() {
  ...
  while (lookahead == VIRGULA) {
    ...
    endereco = searchSymbol(&tabelaSimbolos, id)
                   ->endereco;     // Procura endereço na tabela de símbolos
    printf("\nCRVL %d", endereco); // MEPA - Carrega valor da variável
    printf("\nIMPR");              // MEPA - Instrução de impressão
  }
...
//<expression> ::= <simple_expression> [ <relational_operator> <simple
// expression> ]
void expression() {
  ...
  if (lookahead == IGUAL || lookahead == DIFERENTE || lookahead == MENOR ||
      lookahead == MENOR_IGUAL || lookahead == MAIOR ||
      lookahead == MAIOR_IGUAL || lookahead == AND || lookahead == OR) {
...
//<factor> ::= identifier | constint | constchar | ‘(’ <expression> ‘)’ | not
//<factor> | true | false
void factor() {
  ...
  if (lookahead == IDENTIFICADOR) {
    ...
    endereco =
        searchSymbol(&tabelaSimbolos, id)
            ->endereco; // Procura endereço da variável na tabela de símbolos
    printf("\nCRVL %d", endereco); // MEPA - Carrega valor da variável
    consume(lookahead);
  }

  if (lookahead == NUMERO || lookahead == CARACTER || lookahead == TRUE ||
      lookahead == FALSE) {

    switch (lookahead) {
    case NUMERO:
      printf("\nCRCT %d",
             info_atom.attribute.number); // MEPA - Carrega constante inteira
      break;

    case CARACTER:
      printf("\nCRCT %d",
             info_atom.attribute.ch); // MEPA - Carrega constante caractere
      break;
